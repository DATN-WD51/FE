...
const TicketHourlyTrendChart = () => {
  const { query } = useTableHook("overviewTicket");
  const { data } = useQuery({
    queryKey: [QUERYKEY.STATS, QUERYKEY.STATS.TICKET, "TREND", query],
    queryFn: () => ticketStats.getTicketTrendHourly(query),
  });
...
export default TicketHourlyTrendChart;
